...
/* ==========================================================
   VIDEO RECORD — Frame-by-frame offscreen canvas
   ─────────────────────────────────────────────────────────
   HOW IT WORKS:
   Instead of capturing the live p5 stream (which drops
   frames on slow machines), we:

   1. Pause p5's draw loop.
   2. Create a hidden offscreen <canvas> matching p5's size.
   3. Start MediaRecorder on that offscreen canvas stream.
   4. Drive each frame manually:
        a. Advance noise time t by one step.
        b. Call redraw() so p5 paints the next frame.
        c. Copy p5 canvas → offscreen via drawImage().
        d. Wait 1000/REC_FPS ms so MediaRecorder timestamps
           the frame at the correct rate.
   5. After all frames, stop MediaRecorder, collect the blob,
      show the preview modal, and resume p5's loop.

   Every frame is guaranteed to be captured at full quality.
   Slow hardware just means the recording process takes
   longer — the output video is always smooth.

   Adjust these two constants to taste:
...
function startRecording() {
  ...
  if (!srcCanvas) { isRecording = false; return; }

  /* Offscreen canvas — same size as the p5 canvas */
  ...
  const stream   = offscreen.captureStream(REC_FPS);
  ...
  /* Pause p5 — we will drive frames manually */
  noLoop();

  const totalFrames     = REC_FPS * REC_DURATION_SEC;
  const frameIntervalMs = 1000 / REC_FPS;
  let   frameIndex      = 0;

  function renderNextFrame() {
    if (_recAbort || frameIndex >= totalFrames) {
      recorder.stop();
      isRecording = false;
      return;
    }

    /* Step noise forward one frame */
    t += 0.02 * speed;

    /* Render p5 canvas */
    redraw();

    /* Copy to offscreen so MediaRecorder picks it up */
  ...
  /* Small delay to let MediaRecorder fully initialise */
...
function _showExportPreviewModal({ type, src, filename, onClose }) {
  if (!document.getElementById("fg-modal-style")) {
    ...
    st.innerHTML = `
      ...
      #fg-export-modal .fg-modal-title {
        font-size: 14px; font-weight: 600; color: #111; letter-spacing: 0.3px;
      }
      #fg-export-modal img, #fg-export-modal video {
        width: 100%; border-radius: 8px;
        border: 1px solid rgba(0,0,0,0.12);
        display: block; max-height: 60vh; object-fit: contain; background: #000;
      }
      #fg-export-modal .fg-modal-actions {
        display: flex; gap: 10px; justify-content: flex-end;
      }
      #fg-export-modal .fg-modal-btn {
        padding: 9px 22px; border-radius: 7px; font-size: 13px;
        font-weight: 600; cursor: pointer;
        border: 1px solid rgba(0,0,0,0.18); transition: opacity 0.15s;
      }
      #fg-export-modal .fg-modal-btn:hover { opacity: 0.78; }
      #fg-export-modal .fg-modal-btn-download {
        background: #111; color: #fff; border-color: #111;
      }
      #fg-export-modal .fg-modal-btn-close { background: #fff; color: #111; }
      #fg-export-modal .fg-modal-hint { font-size: 11px; color: #888; margin: 0; }
    `;
    document.head.appendChild(st);
  }
